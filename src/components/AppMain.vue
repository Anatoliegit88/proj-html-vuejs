<script>
export default {
  name: "AppMain",
};
</script>

<template>
  <!-- HERO SECTION -->
  <div class="container mt-5">
    <div class="small-container">
      <div class="title-hero pt-5">
        <h5>
          Let Passion and determination be your guide along the way and develop
          at your own pace that's comfortable.
        </h5>
        <small><strong>FANNIE MORENO</strong></small>
        <small><p>/Founder & CEO</p></small>
      </div>
      <!-- NUMBERS SECTION -->
      <div class="numbers mt-5">
        <div class="text-center">
          <h2><Strong class="numers__color">1.926</Strong></h2>
          <small><h6>FINISHED SESONS</h6></small>
        </div>
        <div class="text-center">
          <h2><Strong>3.092+</Strong></h2>
          <small><h6>ENROLLED ROLLERS</h6></small>
        </div>
        <div class="text-center">
          <h2><Strong>200</Strong></h2>
          <small><h6>ONLINE INSTRUCTORS</h6></small>
        </div>
        <div class="text-center">
          <h2><Strong>100%</Strong></h2>
          <small><h6>SATISFACTION RATE</h6></small>
        </div>
      </div>
      <!-- SERVICE SECTION -->
      <div class="service-section mt-5">
        <div class="left-side">
          <div class="left-side-first">
            <div class="box m-3 shadow p-3">
              <h6 class="box__title"><b>Bussines Administration</b></h6>
              <small
                ><p class="box__parag">
                  Have a passion for graphics and arts? Show your talents with
                  confidence and self-assertiveness.
                </p></small
              >
              <img
                class="img-card"
                src="../assets/images/home-6-service-image-02.png"
                alt="" />
            </div>
            <div class="box m-3 shadow p-3">
              <small
                ><p class="box__parag">
                  Have a passion for graphics and arts? Show your talents with
                  confidence and self-assertiveness.
                </p></small
              >
              <img
                class="img-card"
                src="../assets/images/home-6-service-image-04.png"
                alt="" />
              <h6 class="box__title mt-5"><b>Web Developement</b></h6>
            </div>
          </div>
          <div class="box m-3 shadow p-3">
            <h6 class="box__title"><b>Graphic Design</b></h6>
            <small
              ><p class="box__parag">
                Have a passion for graphics and arts? Show your talents with
                confidence and self-assertiveness.
              </p></small
            >
            <img
              class="img-card mt-5"
              src="../assets/images/home-6-service-image-01.png"
              alt="" />
          </div>

          <div class="box m-3 shadow p-3">
            <small
              ><p class="box__parag">
                Have a passion for graphics and arts? Show your talents with
                confidence and self-assertiveness.
              </p></small
            >
            <img
              class="img-card"
              src="../assets/images/home-6-service-image-03.png"
              alt="" />
            <h6 class="box__title"><b>Idea Discussion</b></h6>
          </div>
        </div>
        <div class="right-side">
          <div class="right-title">
            <p>TOGETHER WE CAN CREATE</p>
            <h4>Services We <strong>Can Provide</strong> For Our Clients</h4>
            <span
              ><small
                ><p class="font-services">
                  <i class="fa-solid fa-check"></i> Select & costumise courses
                  and preferences.
                </p></small
              ></span
            >
            <span class="font-services"
              ><p>
                <i class="fa-solid fa-check"></i> Select & costumise courses and
                preferences.
              </p></span
            >
            <span
              ><p class="font-services">
                <i class="fa-solid fa-check"></i> Select & costumise courses and
                preferences.
              </p></span
            >
            <span
              ><p class="font-services">
                <i class="fa-solid fa-check"></i> Select & costumise courses and
                preferences.
              </p></span
            >
            <button type="button" class="btn btn-success">
              Get started for free
            </button>
          </div>
        </div>
      </div>
      <!-- FEATURES SECTION -->
    </div>
  </div>
  <section class="container-features">
    <div class="features-section">
      <div class="text-center">
        <small><h6 class="h6">CHOOSE A COURSE TO GET STARTED</h6></small>
        <h4><b>Latest Featured </b><strong>Courses</strong></h4>
      </div>
      <div class="section-side">
        <div class="left-side-features">
          <div class="courses d-flex">
            <div>
              <img
                class="b-radius"
                src="../assets/images/course-02-480x298.jpg"
                alt="" />
            </div>
            <div class="courses__price">
              <h5>
                $40.<small><span>00</span></small>
              </h5>
              <h6>Learning to Write a Professional <br />Author</h6>
              <span
                ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                <i class="fa-regular fa-user"></i> 50 Students
              </span>
            </div>
          </div>
          <div class="courses d-flex">
            <div>
              <img
                class="b-radius"
                src="../assets/images/stock-full-hd-05-480x298.jpg"
                alt="" />
            </div>
            <div class="courses__price">
              <h5>
                $40.<small><span>00</span></small>
              </h5>
              <h6>Learning to Write a Professional <br />Author</h6>
              <span
                ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                <i class="fa-regular fa-user"></i> 50 Students
              </span>
            </div>
          </div>
          <div class="courses d-flex">
            <div>
              <img
                class="b-radius"
                src="../assets/images/stock-full-hd-06-480x298.jpg"
                alt="" />
            </div>
            <div class="courses__price">
              <h5>
                $40.<small><span>00</span></small>
              </h5>
              <h6>Learning to Write a Professional <br />Author</h6>
              <span
                ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                <i class="fa-regular fa-user"></i> 50 Students
              </span>
            </div>
          </div>
        </div>
        <div class="right-side-featurese">
          <div class="courses d-flex">
            <div>
              <img
                class="b-radius"
                src="../assets/images/course-featured-image-01-480x298.jpg"
                alt="" />
            </div>
            <div class="courses__price">
              <h5>
                $40.<small><span>00</span></small>
              </h5>
              <h6>Learning to Write a Professional <br />Author</h6>
              <span
                ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                <i class="fa-regular fa-user"></i> 50 Students
              </span>
            </div>
          </div>
          <div class="courses d-flex">
            <div>
              <img
                class="b-radius"
                src="../assets/images/stock-full-hd-03-480x298.jpg"
                alt="" />
            </div>
            <div class="courses__price">
              <h5>
                $40.<small><span>00</span></small>
              </h5>
              <h6>Learning to Write a Professional <br />Author</h6>
              <span
                ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                <i class="fa-regular fa-user"></i> 50 Students
              </span>
            </div>
          </div>
          <div class="courses d-flex">
            <div>
              <img
                class="b-radius"
                src="../assets/images/stock-full-hd-04-480x298.jpg"
                alt="" />
            </div>
            <div class="courses__price">
              <h5>
                $40.<small><span>00</span></small>
              </h5>
              <h6>Learning to Write a Professional <br />Author</h6>
              <span
                ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                <i class="fa-regular fa-user"></i> 50 Students
              </span>
            </div>
          </div>
        </div>
      </div>
      <div class="text-center">
        <button type="button" class="btn btn-success text-center mb-5">
          Get started for free
        </button>
      </div>
    </div>
    <div class="testimonial-list bg-white">
      <div class="text-center p-5">
        <h2>Why people talk about <strong>Maxcouch</strong>?</h2>
        <div class="courses-testimonials d-flex">
          <div>
            <img
              class="b-radius"
              src="../assets/images/testimonial-avata-02.jpg"
              alt="" />
          </div>
          <div class="courses__price text-start w-50">
            <h6>
              I am free to learn to my own peace,folow my schedule and choose
              the subjectI like.Great study portal for people like me.
            </h6>

            <small
              ><strong><p>Mina Hollace</p></strong></small
            >
            <small><p>/Freelance</p></small>
          </div>
        </div>
        <div class="img-brand d-flex mt-5 mb-5 justify-content-evenly">
          <div>
            <img src="../assets/images/client-logo-colored-01.png" alt="" />
          </div>
          <div>
            <img src="../assets/images/client-logo-colored-02.png" alt="" />
          </div>
          <div>
            <img src="../assets/images/client-logo-colored-03.png" alt="" />
          </div>
          <div>
            <img src="../assets/images/client-logo-colored-04.png" alt="" />
          </div>
          <div>
            <img src="../assets/images/client-logo-colored-05.png" alt="" />
          </div>
        </div>
      </div>
    </div>

    <div class="text-center bg-light p-5">
      <small><h6 class="h6">CHOOSE A COURSE TO GET STARTED</h6></small>
      <h3>Latest on <strong>Ouor Blogs</strong></h3>
    </div>
    <div class="our-blogs bg-light d-flex justify-content-evenly">
      <div class="blogs p-5">
        <img src="../assets/images/artist-blog-03-480x325.jpeg" alt="" />
        <small><p>ARTIST</p></small>
        <h6>Brush Strokes Energize Trees in Paintings</h6>
        <span
          ><i class="fa-regular fa-file-lines"></i> 20 Lessons
          <i class="fa-regular fa-user"></i> 50 Students
        </span>
      </div>
      <div class="blogs p-5">
        <img src="../assets/images/artist-blog-02-480x325.jpg" alt="" />
        <small><p>ARTIST</p></small>
        <h6>Brush Strokes Energize Trees in Paintings</h6>
        <span
          ><i class="fa-regular fa-file-lines"></i> 20 Lessons
          <i class="fa-regular fa-user"></i> 50 Students
        </span>
      </div>
      <div class="blogs p-5">
        <img src="../assets/images/artist-blog-01-480x325.jpg" alt="" />
        <small><p>ARTIST</p></small>
        <h6>Brush Strokes Energize Trees in Paintings</h6>
        <span
          ><i class="fa-regular fa-file-lines"></i> 20 Lessons
          <i class="fa-regular fa-user"></i> 50 Students
        </span>
      </div>
      <!-- <div class="blogs p-5">
                <img src="../assets/images/artist-blog-01-480x325.jpg" alt="" />
                <small><p>ARTIST</p></small>
                <h6>Brush Strokes Energize Trees in Paintings</h6>
                <span
                  ><i class="fa-regular fa-file-lines"></i> 20 Lessons
                  <i class="fa-regular fa-user"></i> 50 Students
                </span>
              </div> -->
    </div>
  </section>
  <section class="help">
    <div class="text-center mt-5">
      <small><h6 class="h6">WANNA TRANSFORM YOUR LIFE?</h6></small>
      <h4><b>Glad to </b><strong>Help You</strong><b> Learn</b></h4>
    </div>
    <div class="help-lerning d-flex">
      <div class="blogs p-3">
        <img
          class="help-img"
          src="https://img.icons8.com/dotty/80/null/flag.png" />

        <small
          ><h6><b>Idea Discussion</b></h6></small
        >
        <small
          ><h6 class="h6">
            Get teamed up with the specialists who work and teach coading for
            years at famous universities.
          </h6></small
        >
        <p></p>
      </div>
      <div class="blogs p-3">
        <img
          class="help-img"
          src="https://img.icons8.com/fluency-systems-regular/96/null/settings.png" />
        <small
          ><h6><b>Web Developement</b></h6></small
        >
        <small
          ><h6 class="h6">
            Get teamed up with the specialists who work and teach coading for
            years at famous universities.
          </h6></small
        >
      </div>
      <div class="blogs p-3">
        <img
          class="help-img"
          src="https://img.icons8.com/external-kiranshastry-lineal-kiranshastry/64/null/external-lifesaver-medical-kiranshastry-lineal-kiranshastry.png" />

        <small
          ><h6><b>System Administration</b></h6></small
        >
        <small
          ><h6 class="h6">
            Get teamed up with the specialists who work and teach coading for
            years at famous universities.
          </h6></small
        >
      </div>
      <div class="blogs p-3">
        <img
          class="help-img"
          src="https://img.icons8.com/ios/50/null/imac.png" />

        <small
          ><h6><b>Graphic Design</b></h6></small
        >
        <small
          ><h6 class="h6">
            Get teamed up with the specialists who work and teach coading for
            years at famous universities.
          </h6></small
        >
      </div>
    </div>
    <div class="help-down-img">
      <img src="../assets/images/home-6-services-image.png" alt="" />
    </div>
  </section>
  <section class="certification">
    <div class="text-center mt-5 p-5 bg-white">
      <h5>Start today for getting <strong>Online certification</strong></h5>
      <h3>You can be tour own guiding star with our help!</h3>
      <button type="button" class="btn btn-success mt-3">
        Get started for free
      </button>
    </div>
  </section>
</template>

<style lang="scss" scoped>
// HERO
img {
  width: 100%;
}
.h6 {
  font-size: xx-small;
  color: rgb(130, 130, 130);
}
.title-hero {
  width: 50%;
  text-align: center;
  margin: 0 auto;
}
// NUMBERS
.numbers {
  display: flex;
  justify-content: space-around;
}
strong {
  color: #1fad96;
}
// SERVICE-SECTION
.service-section {
  display: flex;
  .left-side {
    width: 50%;
    position: relative;
    display: block;
    padding-left: 1rem;
    .left-side-first {
      position: absolute;
      top: 5%;
      right: 14%;
      display: block;
    }
    .box {
      border: 1px solid gainsboro;
      border-radius: 10px;
      width: 10rem;
      height: 19rem;
      margin: 0.4rem;
      box-shadow: black;

      .box__title {
        padding-left: 0.5rem;
        padding-top: 1rem;
        font-size: small;
      }
      .box__parag {
        padding-left: 0.5rem;
        font-size: xx-small;
        text-align: start;
      }

      .img-card {
        width: 100%;
      }
    }
  }
}
.right-title {
  text-align: start;
  padding-top: 8rem;
  width: 50%;
  color: black;
}
// FEAUTER-SECTION
.courses-testimonials {
  margin-left: 7rem;
  padding-top: 3rem;
}
.courses__price {
  font-size: xx-small;
}
.font-services {
  font-size: xx-small;
  color: green;
}
.b-radius {
  width: 6rem;
  height: 6rem;
  border-radius: 50%;
  object-fit: cover;
  margin-right: 3rem;
  margin-bottom: 1rem;
}
.container-features {
  background-color: rgb(237, 237, 237);
  padding-top: 2rem;
  margin-top: 2rem;
}
.section-side {
  display: flex;
  flex-wrap: wrap;
  padding: 3rem;
  background-color: rgb(237, 237, 237);
}
.left-side-features {
  width: 50%;
  margin-left: 2rem;
}
.right-side-features {
  width: 50%;
}
// HELP SECTION
.help {
  padding-top: 1rem;
  padding-left: 6rem;
  padding-right: 6rem;
  justify-content: space-around;
  background-color: rgb(237, 237, 237);
  position: relative;
  .help-img {
    width: 3rem;
    padding-bottom: 1rem;
    padding-top: 2rem;
  }
  .help-down-img {
    display: inline-flex;
  }
}
</style>
